...
class socialDb
{
  ...
  function socialDb()
  {
    ...
    $this->friends              = "{$wpdb->prefix}social_friends";
    $this->friend_requests      = "{$wpdb->prefix}social_friend_requests";
    $this->board_posts          = "{$wpdb->prefix}social_board_posts";
    $this->board_comments       = "{$wpdb->prefix}social_board_comments";
    $this->board_post_metas     = "{$wpdb->prefix}social_board_post_metas";
    $this->custom_fields        = "{$wpdb->prefix}social_custom_fields";
    $this->custom_field_values  = "{$wpdb->prefix}social_custom_field_values";
    $this->custom_field_options = "{$wpdb->prefix}social_custom_field_options";
    $this->threads              = "{$wpdb->prefix}social_threads";
    $this->messages             = "{$wpdb->prefix}social_messages";
    $this->message_metas        = "{$wpdb->prefix}social_message_metas";
	$this->category             = "{$wpdb->prefix}social_category";
	$this->product_list         = "{$wpdb->prefix}social_product_list";
	$this->purchase_logs        = "{$wpdb->prefix}social_purchase_logs";
	$this->shopping_cart        = "{$wpdb->prefix}social_shopping_cart";
	$this->photo                = "{$wpdb->prefix}social_photo";
}
  ...
  function upgrade()
  {
    ...
    $old_db_version = get_option('social_db_version');
    ...
    if($db_version != $old_db_version)
    {
      ...
      dbDelta($sql);
	   ...
	   /* Create/Upgrade Product_category Table */
      $sql = "CREATE TABLE {$this->shopping_cart} (
                id int(11) NOT NULL auto_increment,
	  ...
	  $sql = "CREATE TABLE {$this->category} (
                id int(11) NOT NULL auto_increment,
				name varchar(255) NOT NULL,
	  ...
	  $sql = "CREATE TABLE {$this->product_list} (
                id int(11) NOT NULL auto_increment,
     ...
     $sql = "CREATE TABLE {$this->purchase_logs} (
                id int(11) NOT NULL auto_increment,
				user_id int(11) NOT NULL,
    ...
    /***** SAVE DB VERSION *****/
    update_option('social_db_version',$db_version);
  }
  ...
  function after_upgrade($curr_db_version)
  {
    if(isset($curr_db_version) and !empty($curr_db_version))
    {
      if($curr_db_version < 4)
      {
        ...
        if($social_options->display_name_type == 'fullname')
        {
          global $wpdb;
          ...
          {
            $profile = socialUser::get_stored_profile_by_id($user_id);
            if($profile)
            {
      ...
      if($curr_db_version < 9)
      {
        ...
        $query = $wpdb->prepare( $query, 'social_avatar' );
        ...
        foreach( $avatars as $avatar )
        {
          ...
          $user =& socialUser::get_stored_profile_by_id( $avatar->user_id );
          if(isset($user->avatars) and !empty($user->avatars) and is_array($user->avatars))
          {
            ...
            if(!empty($new_avatars))
            {
              ...
              $query = $wpdb->prepare( $query, serialize($new_avatars), $avatar->user_id, 'social_avatars' );
              $wpdb->query( $query );
            }
          }
        }
  ...
  {
    global $wpdb;
    extract(socialDb::get_where_clause_and_values( $args ));
