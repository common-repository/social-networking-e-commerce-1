...
class socialOptions
{
  ...
  var $product_page_id;
  var $product_page_id_str;
  var $checkout_page_id;
  var $checkout_page_id_str;
  var $photo_page_id;
  var $photo_page_id_str;
  var $purchase_history_page_id;
  var $purchase_history_page_id_str;
  ...
  // Is the setup sufficiently completed for social to function?
  var $setup_complete;
  ...
  // Invisible Users -- these guys aren't visible by social
  var $invisible_users;
  ...
  function set_default_options()
  {
    ...
    if(!isset($this->inbox_page_id))
	  ...
	  if(!isset($this->product_page_id))
      $this->product_page_id = 0;
	  
	   if(!isset($this->checkout_page_id))
      $this->checkout_page_id = 0;
	  
	   if(!isset($this->photo_page_id))
      $this->photo_page_id = 0;
	  
	   if(!isset($this->purchase_history_page_id))
      $this->purchase_history_page_id = 0;

    $this->profile_page_id_str         = 'social-profile-page-id';
    $this->profile_edit_page_id_str    = 'social-profile-edit-page-id';
    $this->friends_page_id_str         = 'social-friends-page-id';
    $this->friend_requests_page_id_str = 'social-friend-requests-page-id';
    $this->activity_page_id_str        = 'social-activity-page-id';
    $this->directory_page_id_str       = 'social-directory-page-id';
    $this->login_page_id_str           = 'social-login-page-id';
    $this->signup_page_id_str          = 'social-signup-page-id';
    $this->inbox_page_id_str           = 'social-inbox-page-id';
	$this->product_page_id_str         = 'social-product-page-id';
	$this->checkout_page_id_str        = 'social-checkout_page_id';
	$this->purchase_history_page_id_str= 'social-purchase_history_page_id';
	$this->photo_page_id_str= 'social-photo_page_id';
    ...
    $this->field_visibility_str = "social-field-visibilies";
    ...
    if(!isset($this->mailer))
      $this->mailer = array( 'type'          => 'mail',
                             ...
                             'smtp-username' => '',
                             'smtp-password' => '' );
    $this->mailer_str = 'social_mailer';
    
    if(!isset($this->custom_fields))
      $this->custom_fields = array();
    $this->custom_fields_str = 'social_custom_fields';
  }
  
  function validate($params,$errors)
  {
    $errors = apply_filters( 'social_validate_options', $errors );
  ...
  function update($params)
  {
	...
	$this->update_page('product_page',$params);
	$this->update_page('checkout_page',$params);
	$this->update_page('purchase_history_page',$params);
	$this->update_page('photo_page',$params);
    ...
    do_action( 'social_update_options', $params );
  }
  ...
  function store()
  {
    ...
    if(is_array($this->custom_fields) and !empty($this->custom_fields))
      $social_custom_field->update_fields_from_array($this->custom_fields);

    unset($this->custom_fields); // we just use the array for setting the db

    // Save the posted value in the database
    update_option( 'social_options', $this );
    
    do_action( 'social_store_options' );
  }
  ...
  function set_activity_types()
  {
    ...
    $this->activity_types = apply_filters('social-activity-types', $this->activity_types);
  }
  ...
  function set_notification_types()
  {
    ...
    $this->notification_types = apply_filters('social-notification-types', $this->notification_types);
  }
  ...
  {
    $this->default_friends = apply_filters('social-default-friends', $this->default_friends);
  }
  ...
  function auto_add_page1($page_name)
  {
    return wp_insert_post(array('post_title' => $page_name, 'post_type' => 'page', 'post_status' => 'publish', 'comment_status' => 'closed'));
  }
