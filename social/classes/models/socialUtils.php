...
class socialUtils
{
  ...
  function &php_get_browsercap_ini()
  {
    ...
    if(!isset($browsecap_ini))
    {
      if( version_compare(PHP_VERSION, '5.3.0') >= 0 )
        $browsecap_ini =& parse_ini_file( social_PATH . "/includes/php/php_browsecap.ini", true, INI_SCANNER_RAW );
      else
        $browsecap_ini =& parse_ini_file( social_PATH . "/includes/php/php_browsecap.ini", true );
    }
  ...
  */
  function php_get_browser($agent = NULL)
  {
    ...
    $q_s=array("#\.#","#\*#","#\?#");
    $q_r=array("\.",".*",".?");
    $brows =& socialUtils::php_get_browsercap_ini();
  ...
  function is_robot()
  {
    ...
    $browsecap = socialUtils::php_get_browser($ua_string);
  ...
  { 
    $mystr = socialUtils::RC4($str, socialUtils::wp_salt());
  ...
  function str_decrypt($str) 
  {
    ...
    $mystr = socialUtils::RC4($mystr, socialUtils::wp_salt());
    return $mystr;
  }
...
/* PLUGGABLE FUNCTIONS AS TO NOT STEP ON OTHER PLUGINS' CODE */
  ...
  {
    socialUtils::_include_pluggables('get_currentuserinfo');
    return get_currentuserinfo();
  }

  function &get_userdata($id)
  {
    socialUtils::_include_pluggables('get_userdata');
    return get_userdata($id);
  }

  function get_userdatabylogin($screenname)
  {
    socialUtils::_include_pluggables('get_userdatabylogin');
    return get_userdatabylogin($screenname);
  }

  function wp_mail($recipient, $subject, $message, $header)
  {
    socialUtils::_include_pluggables('wp_mail');
    return wp_mail($recipient, $subject, $message, $header);
  }

  function is_user_logged_in()
  {
    socialUtils::_include_pluggables('is_user_logged_in');
    return is_user_logged_in();
  }

  function get_avatar( $id, $size )
  {
    socialUtils::_include_pluggables('get_avatar');
    return get_avatar( $id, $size );
  }
  
  function wp_hash_password( $password_str )
  {
    socialUtils::_include_pluggables('wp_hash_password');
    return wp_hash_password( $password_str );
  }
  
  function wp_generate_password( $length, $special_chars )
  {
    socialUtils::_include_pluggables('wp_generate_password');
    return wp_generate_password( $length, $special_chars );
  }
  
  function wp_redirect( $location, $status=302 )
  {
    socialUtils::_include_pluggables('wp_redirect');
    return wp_redirect( $location, $status );
  }
  
  function wp_salt( $scheme='auth' )
  {
    socialUtils::_include_pluggables('wp_salt');
