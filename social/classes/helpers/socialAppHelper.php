...
class socialAppHelper
{
  function socialAppHelper()
  {
  }
  ...
  {
    $user = socialUser::get_stored_profile_by_id($user_id, false);
  ...
  function get_avatar_img_by_id($user_id, $size, $filtered_img=false)
  {
    ...
    if($user)
      return socialAppHelper::get_avatar_img($user, $size, $filtered_img);
    else
      return $filtered_img;
  }
  ...
  function time_ago($created_at_ts)
  {
    ...
    if($ago < $minutes)
    {
      $ago_str = sprintf(__ngettext("%d second ago","%d seconds ago", $ago, 'social'), $ago);
    }
    else if($ago < $hours)
    {
      $ago_minutes = (int)($ago / $minutes);
      
      $ago_str = sprintf(__ngettext("%d minute ago", "%d minutes ago", $ago_minutes, 'social'), $ago_minutes);
    }
    else if($ago < $days)
    {
      $ago_hours = (int)($ago / $hours);
      
      $ago_str = sprintf(__ngettext("%d hour ago", "%d hours ago", $ago_hours, 'social'), $ago_hours);
    }
    else if($ago < $weeks)
    {
      $ago_days = (int)($ago / $days);
      
      $ago_str = sprintf(__ngettext("yesterday", "%d days ago", $ago_days, 'social'), $ago_days);
    }
    else if($ago < $months)
    {
      $ago_weeks = (int)($ago / $weeks);
      
      $ago_str = sprintf(__ngettext("last week", "%d weeks ago", $ago_weeks, 'social'), $ago_weeks);
    }
    else if($ago < $years)
    {
      $ago_months = (int)($ago / $months);
      
      $ago_str = sprintf(__ngettext("last month", "%d months ago", $ago_months, 'social'), $ago_months);
    }
    else
    {
      $ago_years = (int)($ago / $years);
      
      $ago_str = sprintf(__ngettext("last year", "%d years ago", $ago_years, 'social'), $ago_years);
    }
  ...
  function powered_by()
  {
    $show_powered_by = apply_filters('social-show-powered-by',true);
    
    if($show_powered_by)
    {
    ?>
      <p style="font-size: 8px;"><?php printf(__('Powered by %1$ssocial%2$s', 'social'), '<span><img src="' . social_URL . '/images/social_16.png" alt="Small social Icon" width="10px" height="10px">&nbsp;<a href="http://blairwilliams.com/social" style="text-decoration: none;">', '</a></span>'); ?></p>
    <?php
    }
  }
