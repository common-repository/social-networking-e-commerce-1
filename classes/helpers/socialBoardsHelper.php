...
class socialBoardsHelper
{
  ...
  { 
    ...
    if( !isset($this_object) or 
      ...
      $this_object =& new socialBoardsHelper();
  ...
  function display_message($message_class, $message, $autoembed_media=true)
  {
    ...
    $formatted_message = socialBoardsHelper::format_message($message, false, $autoembed_media);

    if(strlen($message) >= 255)
    {
      $hidden = ' class="social-hidden"';
      $teaser = socialBoardsHelper::format_message((mb_substr($message,0,255) . "..."), false, false);
      $teaser = socialBoardsHelper::strip_bbcodes($teaser); // get rid of any unmatched bbcodes
      $class_suffix = '-fake';
      
      $teaser_class = $message_class . $class_suffix;
      
      ?><span id="<?php echo $teaser_class; ?>"><?php echo $teaser; ?> <a href="javascript:social_toggle_two_ids('#<?php echo $message_class; ?>','#<?php echo $teaser_class; ?>')"><?php _e('Read More', 'social'); ?></a></span><?php
    }
  ...
  function format_message($message, $strip_newlines=false, $autoembed_media=true)
  {
    ...
    //$message = socialBoardsHelper::parse_textile($message);
    $message = socialBoardsHelper::parse_bbcodes($message);
    ...
    if($autoembed_media)
      $message = socialBoardsHelper::autoembed_media($message);
    */
    
    $message = convert_smilies($message);
    $message = make_clickable($message);
    $message = socialBoardsHelper::make_tags_clickable($message);
    
    $message = apply_filters('social-format-message',$message);
  ...
  function parse_bbcodes($content)
  {
    ...
    $social_boards_helper =& socialBoardsHelper::get_stored_object();
    return preg_replace_callback( '#\[list(=.)?\]\n?(.*?)\[/list\]\n?#is', array(&$social_boards_helper, 'format_bbcode_list'), $content );
  }
  ...
  function escape_code_blocks($content)
  {
    ...
    $social_boards_helper =& socialBoardsHelper::get_stored_object();
    return preg_replace_callback( $code_search, array(&$social_boards_helper, 'escape_code_block_callback'), $content );
  }
  ...
  {
    $social_boards_helper =& socialBoardsHelper::get_stored_object();
    return preg_replace_callback( '#(https?://[^\s"]+)#im', array(&$social_boards_helper, 'autoembed_media_callback'), $message, 1 );
  }
    ...
    if(is_array($matches[1]))
    {
      ...
      {  
        ...
        if( !in_array($username,$usernames) and
            ...
            !in_array($user_id,$social_options->invisible_users) )
        {
          $usernames[] = $username;
          $users[] =& socialUser::get_stored_profile_by_screenname($username);
        }
      }
    }
  ...
  {
    $users =& socialBoardsHelper::get_tagged_users($message);
