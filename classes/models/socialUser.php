...
{ 
  ...
  {
    global $social_options, $current_user, $wpdb;
    socialUtils::get_currentuserinfo();
    
    if( empty($id) )
      $this->userdata =& $current_user;
    else
      $this->userdata =& socialUtils::get_userdata($id);
    ...
    // Set string keys
    $this->profile_name            = 'social_user_profile';
    $this->id_str                  = 'social_user_id';
    $this->avatar_str              = 'social_user_avatar';
    $this->first_name_str          = 'social_user_first-name';
    $this->last_name_str           = 'social_user_last-name';
    $this->screenname_str          = 'social_user_screenname';
    $this->email_str               = 'social_user_email';
    $this->url_str                 = 'social_user_url';
    $this->location_str            = 'social_user_location';
    $this->sex_str                 = 'social_user_sex';
    $this->bio_str                 = 'social_user_bio';
    $this->birthday_str            = 'social_user_birthday';
    $this->fields_str              = 'social_user_fields';
    $this->hide_notifications_str  = 'social_user_hide_notifications';
    $this->privacy_str             = 'social_user_privacy';
    $this->password_str            = 'social_password';
    $this->password_confirm_str    = 'social_password_confirm';
    $this->user_status_str         = 'social-user-status';
    $this->user_status_time_str    = 'social-user-status-time';
    $this->user_status_time_ts_str = 'social-user-status-time-ts';
    ...
    $this->avatar             = $this->_get_metadata( 'social_avatar' );
    $this->avatars            = $this->_get_metadata( 'social_avatars' );
    $this->location           = $this->_get_metadata( 'social_location' );
    $this->sex                = $this->_get_metadata( 'social_sex' );
    $this->fields             = $this->_get_metadata( 'social_fields' );
    ...
    $date_format = __('F j, Y', 'social');
    
    $birthday_timestamp = (int)$this->_get_metadata( 'social_birthday' );
    ...
    else
    {
          ...
          (isset($avmeta[1]) && (int)$avmeta[1] > 600) )
      {
        ...
        if ( !is_wp_error($resized_image) and $resized_image )
        {
          ...
          $this->_update_metadata( 'social_avatar', $this->avatar );
        }
      }
    }
    ...
    if($this->sex == 'female')
    {
      $this->his_her = __('Her', 'social');
      $this->him_her = __('Her', 'social');
      $this->he_she  = __('She', 'social');
      $this->sex_display = __('Female', 'social');
    }
    else if($this->sex == 'male')
    {
      $this->his_her = __('His', 'social');
      $this->him_her = __('Him', 'social');
      $this->he_she  = __('He', 'social');
      $this->sex_display = __('Male', 'social');
    }
    else
    {
      $this->his_her = __('His/Her', 'social');
      $this->him_her = __('Him/Her', 'social');
      $this->he_she  = __('He/She', 'social');
      $this->sex_display = __('Not Available', 'social');
    }
        ...
        foreach($this->avatars as $type)
        {
          foreach($type as $sized)
          {
            ...
            if( file_exists($this->_avatar_path() . "/" . $avatar_guess) and 
                socialUtils::is_image($this->_avatar_path() . "/" . $avatar_guess) )
            {
  ...
  function validate_signup($params,$errors)
  {
    ...
    if($social_options->signup_robot_protection and socialUtils::is_robot())
      $errors[] = __('This site prevents robots from registering and you don\'t appear to be human. If you\'re a real person, please contact the administrator of this site.', 'social');

    if(empty($user_login))
      $errors[] = __('Username must not be blank','social');

    if(!preg_match('#^[a-zA-Z0-9_]+$#',$user_login))
      $errors[] = __('Username must only contain letters, numbers and/or underscores','social');
    
    require_once(ABSPATH . WPINC . '/registration.php');
    $user_id = username_exists( $user_login );
    $available = socialUtils::username_is_available( $user_login );
    if ( $user_id or !$available)
    	$errors[] = __('Username is Already Taken.','social');
    
    if(empty($user_email))
      $errors[] = __('Email must not be blank','social');
      
    if(!is_email($user_email, $social_options->signup_spam_email_protection))
      $errors[] = __('Email must be a real and properly formatted email address','social');
      
    if(email_exists($user_email))
      $errors[] = __('Email Address has already been used by another user.','social');

    if(isset($social_options->field_visibilities['signup_page']['sex']) and empty($social_user_sex))
      $errors[] = __('You must select your Gender.','social');

    // Captcha
    if($social_options->signup_captcha)
    {
      $captcha_code = socialUtils::str_decrypt($security_check);
      if(!empty($captcha_code) and $captcha_code != $security_code)
        $errors[] = __('Captcha Image Verification Was Incorrect, Please Try Again','social');
    }
      
    if(isset($social_options->field_visibilities['signup_page']['password']))
    {
      if(empty($social_user_password))
        $errors[] = __('You must enter a Password.','social');
        
      if(empty($social_user_password_confirm))
        $errors[] = __('You must enter a Password Confirmation.', 'social');
        
      if($social_user_password != $social_user_password_confirm)
        $errors[] = __('Your Password and Password Confirmation don\'t match.', 'social');
    }
  ...
  function validate_login($params,$errors)
  {
    ...
    if(!username_exists($log))
      $errors[] = __('Username was not found','social');
    else
    {
      ...
      if(!user_pass_ok($log, $pwd))
        $errors[] = __('Your Password was Incorrect','social');
    }
  ...
  function validate_forgot_password($params,$errors)
  {
    ...
    if(empty($social_user_or_email))
      $errors[] = __('You must enter a Username or Password','social');
    else
    {
      ...
      if(!$is_email and !$is_username)
        $errors[] = __('That Username or Email wasn\'t found.','social');
    }
  ...
  {   
    ...
    {
      $errors[] = __("Password Must Match Password Confirmation.", 'social');
    }
    ...
    {
      $errors[] = __("Your Avatar wasn't able to be saved.", 'social');
    }
    ...
    if( isset($_FILES[ $this->avatar_str ]) and 
        !empty($_FILES[ $this->avatar_str ]['name']) and
        (int)$_FILES[ $this->avatar_str ]['size'] > 0 and
        !socialUtils::is_image($_FILES[ $this->avatar_str ]['tmp_name']))
    {
      $errors[] = __("Your Avatar must be a valid jpg, gif or png.", 'social');
    }
  ...
  function update($params)
  {
    ...
    { 
      ...
      if( isset( $params[ $this->password_str ] ) and
          ...
          ( $params[ $this->password_str ] == $params[ $this->password_confirm_str ] ) )
      {
        $this->hashed_password = socialUtils::wp_hash_password( $params[ $this->password_str ] );
      }
          ...
          (int)$_FILES[ $this->avatar_str ]['size'] > 0 )
      {
        ...
        $target_path = $target_path . "/social";
        if(!file_exists($target_path))
          @mkdir($target_path."/");
        ...
        $ext = socialAppHelper::get_extension($avatar_meta['mime']); 
  ...
  function _avatar_path()
  {
    ...
    return "{$avatar_path}/social/avatars";
  }
  ...
  function _avatar_url()
  {
    ...
    return "{$avatar_url}/social/avatars";
  }
  ...
  function delete_avatars()
  {
    if(isset($this->avatar) and !empty($this->avatar))
    {
      ...
      unset($this->avatar);
      delete_usermeta( $this->id, 'social_avatar' );
    }
  ...
  function delete_sized_avatars()
  {
    if(isset($this->avatars) and !empty($this->avatars) and is_array($this->avatars))
    {
      ...
      unset($this->avatars);
      delete_usermeta( $this->id, 'social_avatars' );
    }
  }
  ...
  function store($force = false)
  {
    ...
    {
      wp_cache_delete("social_user", "{$this->id}"); // Clear the cache
      ...
      $this->_update_metadata( 'social_avatar',            $this->avatar );
      $this->_update_metadata( 'social_location',          $this->location );
      $this->_update_metadata( 'social_sex',               $this->sex );
      $this->_update_metadata( 'description',            $this->bio );
      $this->_update_metadata( 'social_birthday',          $this->birthday );
      $this->_update_metadata( 'social_fields',            $this->fields );
  ...
  {
    global $wpdb, $social_friend, $user_id;
  ...
  {
    $avatar = socialUtils::get_avatar( $this->id, $size );
    $avatar = socialAppHelper::add_avatar_class($avatar, 'social-profile-image');
    $avatar = socialAppHelper::link_avatar( $this->id, $avatar );
  ...
  function get_sized_avatar($size,$square=true)
  {
        ...
        !file_exists($this->_avatar_path() . "/" . $this->avatars[$type][$size]) )
    {
      ...
      if( !is_wp_error($resized_avatar) and $resized_avatar )
      {
        ...
        $this->_update_metadata( 'social_avatars', $this->avatars );
      }
      else
      {
  ...
  function get_profile_url()
  {
    ...
    if(isset($social_options->profile_page_id) and $social_options->profile_page_id != 0)
    {
      if( socialUtils::rewriting_on() and $social_options->pretty_profile_urls )
      {
        global $social_blogurl;
        $struct = socialUtils::get_permalink_pre_slug_uri();
        return "{$social_blogurl}{$struct}{$this->screenname}";
      }
      else
      {
        $permalink = get_permalink($social_options->profile_page_id);
  ...
  function get_friends_url()
  {
    ...
    if(isset($social_options->friends_page_id) and $social_options->friends_page_id != 0)
    {
      $permalink = get_permalink($social_options->friends_page_id);
      $param_char = ((preg_match("#\?#",$permalink))?'&':'?');
      return "{$permalink}{$param_char}u={$this->screenname}";
    }
  ...
  { 
    return socialUser::get_stored_profile($user_id, $load_default);
  }
  ...
  function &get_stored_profile_by_screenname($screenname = '', $load_default = true)
  {
    ...
    if(!empty($screenname))
      $user = socialUtils::get_userdatabylogin($screenname);
    
    return socialUser::get_stored_profile($user->ID, $load_default);
  }
  ...
  { 
    ...
    if((empty($user_id) and $load_default) or
       (!is_admin() and in_array($user_id, $social_options->invisible_users)))
    {
      global $current_user;
      socialUtils::get_currentuserinfo();
    ...
    {
      $user = wp_cache_get("social_user", "{$user_id}");
      if($user == false)
      {
      	$user = new socialUser($user_id);
      	$expire = 24 * 60 * 60; // 1 whole day
      	wp_cache_set("social_user", $user, "{$user_id}", $expire);
    	}
  ...
  function get_stored_profiles($search_query='', $offset=0, $limit=50, $order_by='user_login ASC')
  {
    ...
    if((!is_admin() and !empty($social_options->invisible_users)) or !empty($search_query))
    {
      $where .= "WHERE ";

      // Hide invisibles
      if(!is_admin() and !empty($social_options->invisible_users))
        $where .= "ID NOT IN (" . implode(',',$social_options->invisible_users) . ")";
      
      if((!is_admin() and !empty($social_options->invisible_users)) and !empty($search_query))
        $where .= " AND ";
    ...
    {
      $curr_profile = socialUser::get_stored_profile_by_id($user_id);
  ...
  function get_count($add_where='')
  {
    ...
    if(!empty($social_options->invisible_users) or !empty($add_where))
    {
      $where .= " WHERE ";
      ...
      if((!is_admin() and !empty($social_options->invisible_users)) and !empty($add_where))
        $where .= " AND ";
  ...
  function is_logged_in_and_visible()
  {
    ...
    return (socialUtils::is_user_logged_in() and socialUser::user_exists_and_visible($current_user->ID));
  }
  ...
  function is_logged_in_and_current_user($user_id)
  {
    ...
    return (socialUser::is_logged_in_and_visible() and ($current_user->ID == $user_id));
  }
  
  function is_logged_in_and_an_admin()
  {
    return (socialUtils::is_user_logged_in() and socialUser::is_admin());
  }
  ...
  {
    return socialUser::get_stored_profile_by_id($id, false);
  }
  ...
  function user_exists_and_visible($user_id)
  {
    ...
    if(socialUser::get_stored_profile_by_id($user_id))
      return !in_array($user_id, $social_options->invisible_users);
    else
      return false;
  }
  ...
  function screenname_exists($screenname)
  {
    ...
    if(!empty($screenname))
      $user = socialUser::get_stored_profile_by_screenname($screenname, false);
  ...
  function screenname_exists_and_visible($screenname)
  {
    ...
    if($user_id = socialUser::screenname_exists($screenname))
      return !in_array($user_id, $social_options->invisible_users);
    else
      return false;
  }
  ...
  {
    global $social_blogname, $social_blogurl, $social_options;
    
    $login_link = get_permalink($social_options->login_page_id);
    
    if(empty($login_link))
      $login_link = $social_blogurl;
    ...
    $subject       = sprintf( __("[%s] New User Registration",'social'), $social_blogname);
    ...
    $message       = sprintf( __( "A new user just joined your community at %1\$s!\n\nName: %2\$s\nUsername: %3\$s\nE-Mail: %4\$s\n\nYou can view this user's profile here: %5\$s", 'social' ), $social_blogname, $this->full_name, $this->screenname, $this->email, $this->get_profile_url() );
    $message      .= "\n\n";
    $message      .= sprintf( __( "There are now %1\$s Users on %2\$s" , 'social'), number_format( (float)$this->get_count() ), $social_blogname );
    
    socialUtils::wp_mail($recipient, $subject, $message, $header);
    ...
    /* translators: In this string, %s is the Blog Name/Title */
    $subject       = sprintf( __("Welcome to %s!",'social'), $social_blogname);
    ...
    $message       = sprintf( __( "%1\$s,\nWelcome to the Community at %2\$s!\n\nUsername: %3\$s\nPassword: %4\$s\nYou can login here: %5\$s\n\nEnjoy!\n\n%2\$s Team", 'social' ), (empty($this->first_name)?$this->screenname:$this->first_name), $social_blogname, $this->screenname, $password, $login_link );
  ...
  {
    $stored_key = $this->_get_metadata( 'social_reset_password_key' );
  ...
  function send_reset_password_requested_notification()
  {
    ...
    $this->_update_metadata( 'social_reset_password_key', $key );
    ...
    /* translators: In this string, %s is the Blog Name/Title */
    $subject       = sprintf( __("[%s] Password Reset",'social'), $social_blogname);
  ...
  {  
    ...
    {
      $this->_delete_metadata( 'social_reset_password_key' );

      //$password = socialUtils::wp_generate_password( 12, false );
      $this->hashed_password = socialUtils::wp_hash_password( $password );
      $this->store(true);

      $edit_permalink = get_permalink($social_options->profile_edit_page_id);
      ...
      /* translators: In this string, %s is the Blog Name/Title */
      $subject       = sprintf( __("[%s] Password Lost/Changed",'social'), $social_blogname);

      /* translators: In this string, %1$s is the user's screenname and %2$s is their profile url*/
      $message       = sprintf( __( "Password Lost and Changed for user: %1\$s\nYou can view %1\$s's profile here: %2\$s", 'social' ), $this->screenname, $this->get_profile_url() );
      ...
      /* translators: In this string, %s is the Blog Name/Title */
      $subject       = sprintf( __("[%s] Your new password",'social'), $social_blogname);

      /* translators: In this string, %1$s is the user's first name, %2$s is the blog's name/title, %3$s is the user's username, %4$s is the user's password, and %5$s is the blog's URL... */
      $message       = sprintf( __( "%1\$s,\n\nYour Password was successfully reset on %2\$s!\n\nUsername: %3\$s\nPassword: %4\$s\n\nYou can login here: %5\$s", 'social' ), (empty($this->first_name)?$this->screenname:$this->first_name), $social_blogname, $this->screenname, $password, $login_link );
