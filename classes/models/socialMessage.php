...
class socialMessage
{
  ...
  {
    global $wpdb;
    $this->table_name         = "{$wpdb->prefix}social_messages";
    $this->threads_table_name = "{$wpdb->prefix}social_threads";
  ...
  {
    $notification_types['private_message'] = array( 'name'        => __('Private Message', 'social'),
                                                    'description' => __('Sent when someone sends you a message', 'social') );

    $notification_types['private_message_reply'] = array( 'name'        => __('Private Message Reply', 'social'),
                                                          'description' => __('Sent when someone replies to a message you were included on', 'social') );
  ...
  function get_all_messages_by_thread_id($thread_id, $mark_as_read=false)
  {
    ...
    $query = $wpdb->prepare($query, $thread_id, $social_user->id);
    return $wpdb->get_results($query);
  }
  ...
  function get_latest_messages($pagenum=1,$pagesize=5)
  {
    ...
    $query = "SELECT mpm.*, UNIX_TIMESTAMP(mpm.created_at) as created_at_ts
                ...
                {$limit}";
    $query = $wpdb->prepare($query, $social_user->id);
  ...
  {
    global $social_user, $wpdb;
    $query = "UPDATE {$this->table_name} SET unread=%d WHERE recipient_id=%d AND thread_id=%d";
    $query = $wpdb->prepare($query, $unread, $social_user->id, $thread_id);
  ...
  function delete_messages($thread_id)
  {
    ...
    $query = $wpdb->prepare( $query, $social_user->id, $thread_id );
  ...
  function validate($params, $ids, $errors)
  {
    ...
    foreach($ids as $id)
    {
      if (!$social_friend->is_friend($social_user->id, $id) && $social_user->id != $id && !$social_user->is_admin())
        $errors[] = __('You can only send messages to your friends', 'social');
    }
    
    $count = count($ids); //Prevents a user from sending a message to themselves
    if(empty($social_message_recipients) || $count == 1)
      $errors[] = __('There must be at least one valid recipient', 'social');
      
    if(empty($social_message_body))
      $errors[] = __('Message can\'t be blank', 'social');
    
    if(empty($social_message_subject))
      $errors[] = __('Subject can\'t be blank', 'social');
  ...
  function get_composer_url()
  {
    ...
    $url = get_permalink($social_options->inbox_page_id);
  ...
  function get_messages_url()
  {
    ...
    return get_permalink($social_options->inbox_page_id);
  }

  function message_notification( $to_id, $thread_id, $subject, $body )
  {
    global $social_options, $social_blogname, $social_user;

    $to = socialUser::get_stored_profile_by_id($to_id);

    if($to)
    {
      $message_url = get_permalink($social_options->inbox_page_id);
      $message_url .= ((preg_match('#\?#',$message_url))?"&":"?") . "action=view&t={$thread_id}";

      $opener = sprintf(__('%s sent you a message.', 'social'), $social_user->screenname );
      $closer = sprintf(__("To reply to this message, follow the link below:\n%s", 'social'), $message_url );
...
{$closer}
MAIL_BODY;
      $subject = sprintf(__('%1$s sent you a message on %2$s...', 'social'), $social_user->screenname, $social_blogname); //subject

      socialNotification::send_notification_email($to, $subject, $mail_body, 'private_message');
    }
  }
  ...
  function message_reply_notification( $to_id, $thread_id, $subject, $body )
  {
    ...
    if($to)
    {
      ...
      $opener = sprintf(__('%s sent a message in reply to a thread.', 'social'), $social_user->screenname );
      $closer = sprintf(__("To reply to this message, follow the link below:\n%s", 'social'), $message_url );
...
{$closer}
MAIL_BODY;
      $subject = sprintf(__('%1$s replied to a thread on %2$s...', 'social'), $social_user->screenname, $social_blogname); //subject

      socialNotification::send_notification_email($to, $subject, $mail_body, 'private_message_reply');
    }
  }
