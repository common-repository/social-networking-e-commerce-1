...
class socialFriend
{
  ...
  {
    global $wpdb;
    $this->table_name = "{$wpdb->prefix}social_friends";
    $this->requests_table_name = "{$wpdb->prefix}social_friend_requests";
  }
  ...
  {
    global $wpdb, $social_friend, $social_options;
    
    if(in_array($user_id,$social_options->invisible_users))
      return false;
    ...
    $query = "SELECT wpmf.*, wpu.* FROM {$social_friend->table_name} wpmf JOIN {$wpdb->users} wpu ON wpmf.friend_id=wpu.ID WHERE wpmf.user_id={$user_id}{$inv_str}{$where}{$order_by}{$limit}";
  ...
  function get_friends_user_array( $user_id, $where='', $offset=0, $limit=50, $order_by='' )
  {
    ...
    if( socialFriend::get_friend_count( $user_id, "status='verified'{$where}" ) <= 0 )
      return false;
    ...
    $friends = socialFriend::get_all_by_user_id( $user_id, "status='verified'{$where}", $order_by, $limit_stmt );

    $friends_a = array();
    
    foreach ($friends as $friend) 
    {
      $new_friend  = socialUser::get_stored_profile_by_id($friend->friend_id);
  ...
  function get_friend_count( $user_id, $where='' )
  {
    ...
    $query = "SELECT COUNT(*) FROM {$social_friend->table_name} wpmf JOIN {$wpdb->users} wpu ON wpmf.friend_id=wpu.ID WHERE wpmf.user_id={$user_id}{$inv_str}{$where}";
  ...
  {
    return socialFriend::is_friend($user_id, $friend_id, 'pending');
  }
  ...
  function is_friend($user_id, $friend_id, $status='verified')
  {
    ...
    {
      global $wpdb, $social_friend;

      $query = "SELECT id FROM {$social_friend->table_name} WHERE user_id=%d AND friend_id=%d AND status=%s";
  ...
  function accept_friend($request_id)
  {
    ...
    socialNotification::friendship_verified($request->friend_id, $request->user_id);
  }
  ...
  {  
    ...
    {   
      ...
      if($query_results)
      {
        socialNotification::friendship_requested($user_id, $friend_id);
  ...
  {
    return apply_filters('social-can-friend', true, $user_id, $friend_id);
  }
  
  function can_delete_friend($user_id, $friend_id)
  {
    return apply_filters('social-can-delete-friend', true, $user_id, $friend_id);
  }
}
?>
